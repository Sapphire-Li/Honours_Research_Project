...
p1 <- comb |> ggplot(aes(w, pool_train)) +
  ...
  # annotate("text", x = optimal[[1]], y = optimal[[2]],
  #        label = paste0("Max: ", round(optimal[[2]],4)), vjust = 3, hjust = 0.8, size = 4) +
  # annotate("text", x = optimal[[1]], y = optimal[[2]],
  #          label = paste0("Optimal Weight: ", round(optimal[[1]],4)), vjust = 5, hjust = 0.8, size = 4)
...
p2 <- comb |> ggplot(aes(w, pool_test)) +
  ...
  theme(plot.title = element_text(hjust = 0.5),
        title = element_text(size = 9),
        axis.text = element_text(size = 6)) +
  geom_point(aes(x = optimal[[1]], y =  opt), size = 3, color = "orange") +
  geom_point(aes(x = 0.5, y = equ), size = 3, color = "blue") +
  ...
  # annotate("text", x = optimal[[1]], y = opt, 
  #          label = paste0("Optimal Weight: ", round(optimal[[1]],4)), vjust = 3, hjust = 0.8, size = 4) +
  # annotate("text", x = optimal[[1]], y = opt,
  #          label = paste0("LPS: ", round(opt[[2]],4)), vjust = 5, hjust = 0.8, size = 4) +
  # annotate("text", x = 0.5, y = equ,
