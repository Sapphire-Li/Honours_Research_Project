...
sd_1_train <- glance(fit_1) |> select(sigma2) |> sqrt() |> as.numeric()
pd_1_train <- numeric(R-1) |> as.numeric()
for (j in 1:(R-1)) {
  pd_1_train[j] <- dlnorm(train[[j+1,2]], mean_1_train[[j]], sd_1_train[[1]])
}
# individual predictive density (in-sample period)
LS_1_train <- sum(log(pd_1_train))
LS_1_train
# -5722.662
...
var_2_train <- glance(fit_2) |> select(sigma2) |> as.numeric()
var_2_train <- mean_2_train^2*var_2_train
sd_2_train <- sqrt(var_2_train)
pd_2_train <- numeric(R) |> as.numeric()
for (j in 1:R) {
  pd_2_train[j] <- dnorm(train[[j,2]], mean_2_train[j], sd_2_train[j])
}
# individual predictive density (in-sample period)
...
level[R+1] <- level[R]*(1 + fit_2_coef[1,2]*resid_2_train[R,1])
for(i in 1:P) {
  resid_2[i] <- (test[i,2] - level[R+i]) / level[R+i]
  level[R+i+1] <- level[R+i]*(1 + fit_2_coef[1,2]*resid_2[i])
}
...
resid_3_train <- residuals(fit_3) |> as_tibble() |> select(.resid)
level <- numeric(T+1) |> as.numeric()
level[1] <- fit_3_coef[3,2] # l_0
trend <- numeric(T+1) |> as.numeric()
trend[1] <- fit_3_coef[4,2] # b_0

# l_1+b_1 ... l_(t-1)+b_(t-1) : expectation of y_1 - y_t
for (j in 1:(R-1)) {
  level[j+1] <- (level[[j]] + trend[[j]])*(1 + fit_3_coef[1,2]*resid_3_train[j,1])
  trend[j+1] <- trend[[j]] + fit_3_coef[1,2]*fit_3_coef[2,2]*(level[[j]] + trend[[j]])*resid_3_train[j,1]
}
mean_3_train <- numeric(R) |> as.numeric()
for (j in 1:R) {
  mean_3_train[j] <- level[[j]] + trend[[j]]
}
var_3_train <- glance(fit_3) |> select(sigma2) |> as.numeric()
var_3_train <- mean_3_train^2*var_3_train
sd_3_train <- sqrt(var_3_train)
pd_3_train <- numeric(R) |> as.numeric()
for (j in 1:R) {
  pd_3_train[j] <- dnorm(train[[j,2]], mean_3_train[j], sd_3_train[j])
}
# individual predictive density (in-sample period)
LS_3_train <- sum(log(pd_3_train))
LS_3_train
# -6474.619
...
weight_optimal <- numeric(4+3+2+1) |> as.numeric()
LS_comb_optimal <- numeric(4+3+2+1) |> as.numeric()
...
comb <- cbind(w,pool_train) |> as_tibble()
comb |> filter(pool_train == max(comb$pool_train))
weight_optimal[4] <- comb |> filter(pool_train == max(comb$pool_train)) |> select(w) |> as.numeric()
LS_comb_optimal[4] <- comb |> filter(pool_train == max(comb$pool_train)) |> select(pool_train) |> as.numeric()
...
comb <- cbind(w,pool_train) |> as_tibble()
comb |> filter(pool_train == max(comb$pool_train))
weight_optimal[5] <- comb |> filter(pool_train == max(comb$pool_train)) |> select(w) |> as.numeric()
LS_comb_optimal[5] <- comb |> filter(pool_train == max(comb$pool_train)) |> select(pool_train) |> as.numeric()
...
comb <- cbind(w,pool_train) |> as_tibble()
comb |> filter(pool_train == max(comb$pool_train))
weight_optimal[6] <- comb |> filter(pool_train == max(comb$pool_train)) |> select(w) |> as.numeric()
LS_comb_optimal[6] <- comb |> filter(pool_train == max(comb$pool_train)) |> select(pool_train) |> as.numeric()
...
comb <- cbind(w,pool_train) |> as_tibble()
comb |> filter(pool_train == max(comb$pool_train))
weight_optimal[7] <- comb |> filter(pool_train == max(comb$pool_train)) |> select(w) |> as.numeric()
LS_comb_optimal[7] <- comb |> filter(pool_train == max(comb$pool_train)) |> select(pool_train) |> as.numeric()
...
comb <- cbind(w,pool_train) |> as_tibble()
comb |> filter(pool_train == max(comb$pool_train))
weight_optimal[8] <- comb |> filter(pool_train == max(comb$pool_train)) |> select(w) |> as.numeric()
LS_comb_optimal[8] <- comb |> filter(pool_train == max(comb$pool_train)) |> select(pool_train) |> as.numeric()
...
comb <- cbind(w,pool_train) |> as_tibble()
comb |> filter(pool_train == max(comb$pool_train))
weight_optimal[9] <- comb |> filter(pool_train == max(comb$pool_train)) |> select(w) |> as.numeric()
LS_comb_optimal[9] <- comb |> filter(pool_train == max(comb$pool_train)) |> select(pool_train) |> as.numeric()
...
comb <- cbind(w,pool_train) |> as_tibble()
comb |> filter(pool_train == max(comb$pool_train))
weight_optimal[10] <- comb |> filter(pool_train == max(comb$pool_train)) |> select(w) |> as.numeric()
LS_comb_optimal[10] <- comb |> filter(pool_train == max(comb$pool_train)) |> select(pool_train) |> as.numeric()
...
p1 <- p1 + scale_y_continuous(limits = c(-5920, -5838))
p2 <- p2 + scale_y_continuous(limits = c(-5920, -5838))
p7 <- p7 + scale_y_continuous(limits = c(-5920, -5838))
p9 <- p9 + scale_y_continuous(limits = c(-5920, -5838))
