...
Specifically, we only consider the combination of two individual forecasts, which allows us to delve into interesting and unexplained findings through fast and relative simple data manipulation.
...
\begin{equation}
\label{eqn:LS1}
LS = \sum^R_{t=1} log \ f(y_t| \mathcal{F}_{t-1}, \hat\theta_M, M).
\end{equation}
...
\begin{equation}
\label{eqn:LS2}
\hat{\omega}_{\text{opt}} =  \underset{\omega \in [0,1]}{\arg\max} \sum^R_{t=1} log \Big[ \omega \ f_1(y_t| \mathcal{F}_{t-1}, \hat\theta_{M1}, M_1) + (1-\omega) \ f_2(y_t| \mathcal{F}_{t-1}, \hat\theta_{M2}, M_2)\Big].
\end{equation}
...
where $\omega$ is the non-negative weight allocated to the point forecast generated from the first model.
...
The lower the MSE, the higher the accuracy of the forecast. Therefore, the "optimal" set of weights satisfies that it minimizes the MSE of the point forecast combination among all other possible sets over the training period:
