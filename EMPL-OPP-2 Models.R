...
for (j in 1:R) {
  SE_2_fit[j] <- (mean_2_train[[j]] - train$Log[j])^2
}
...
weight_optimal_1 <- comb |> filter(pool_train == min(comb$pool_train)) |> select(w) |> as.numeric()
LS_comb_optimal_1 <- comb |> filter(pool_train == min(comb$pool_train)) |> select(pool_train) |> as.numeric()

p1 <- comb |> ggplot(aes(w, pool_train)) +
  geom_line(color = "red") +
  labs(
    title = "The point combination between SARIMA and ETS(A,A,A) - Correct Specification",
  ...
  annotate("text", x = weight_optimal_1, y = LS_comb_optimal_1,
           label = paste0("Min: ", round(LS_comb_optimal_1,9)), vjust = -2, size = 4) +
  annotate("text", x = weight_optimal_1, y = LS_comb_optimal_1,
           label = paste0("Weight: ", round(weight_optimal_1,4)), vjust = -4, size = 4) +
  geom_point(aes(x = weight_optimal_1, y = LS_comb_optimal_1), size = 3, color = "orange")
...
comb <- cbind(w, pool_train) |> as_tibble()
comb |> filter(pool_train == min(comb$pool_train))
weight_optimal_2 <- comb |> filter(pool_train == min(comb$pool_train)) |> select(w) |> as.numeric()
LS_comb_optimal_2 <- comb |> filter(pool_train == min(comb$pool_train)) |> select(pool_train) |> as.numeric()

p3 <- comb |> ggplot(aes(w, pool_train)) +
  geom_line(color = "red") +
  labs(
    title = "The point combination between ARIMA(2,1,0) w/ drift and ETS(A,A,N) - Misspecification",
  ...
  theme(plot.title = element_text(hjust = 0.5, size = 10),
        title = element_text(size = 12),
        axis.text = element_text(size = 9)) +
  annotate("text", x = weight_optimal_2, y = LS_comb_optimal_2,
           label = paste0("Min: ", round(LS_comb_optimal_2,9)), vjust = -2, size = 4) +
  annotate("text", x = weight_optimal_2, y = LS_comb_optimal_2,
           label = paste0("Weight: ", round(weight_optimal_2,4)), vjust = -4, size = 4) +
  geom_point(aes(x = weight_optimal_2, y = LS_comb_optimal_2), size = 3, color = "orange")
...
LS_comb_2 <- comb |> filter(w == weight_optimal_1) |> select(pool) |> as.numeric()
...
p2 <- comb |> ggplot(aes(w, pool)) +
  ...
  theme(plot.title = element_text(hjust = 0.5),
        title = element_text(size = 12),
        axis.text = element_text(size = 9)) +
  geom_point(aes(x = weight_optimal_1, y = LS_comb_2), size = 2, color = "orange") +
  ...
  annotate("text", x = weight_optimal_1, y = LS_comb_2,
           label = paste0("Optimal Weight: ", round(weight_optimal_1,4)), vjust = 2, hjust = 1.1, size = 4) +
  annotate("text", x = weight_optimal_1, y = LS_comb_2,
           label = paste0("MSFE: ", round(LS_comb_2,6)), vjust = 4, hjust = 1.1, size = 4) +
  annotate("text", x = 0.5, y = equ_1,
           label = paste0("Simple Average: ", round(equ_1,6)), vjust = -1, hjust = -0.1, size = 4)
...
weight_test <- comb |> filter(pool == min(comb$pool)) |> select(w) |> as.numeric()
LS_comb_test <- comb |> filter(pool == min(comb$pool)) |> select(pool) |> as.numeric()
LS_comb_4 <- comb |> filter(w == weight_optimal_2) |> select(pool) |> as.numeric()
...
p4 <- comb |> ggplot(aes(w, pool)) +
  ...
  theme(plot.title = element_text(hjust = 0.5),
        title = element_text(size = 12),
        axis.text = element_text(size = 9)) +
  geom_point(aes(x = weight_optimal_2, y = LS_comb_4), size = 2, color = "orange") +
  # geom_point(aes(x = weight_test, y = LS_comb_test), size = 1.5, color = "green") +
  ...
  # annotate("text", x = weight_test, y = LS_comb_test,
  #          label = paste0("Weight: ", round(weight_test,4)), vjust = -5, hjust = 0.8, size = 3) +
  annotate("text", x = weight_optimal_2, y = LS_comb_4,
           label = paste0("Optimal Weight: ", round(weight_optimal_2,4)), hjust = 1.1, size = 4) +
  annotate("text", x = weight_optimal_2, y = LS_comb_4,
...
pdf("EMPL.pdf", width = 13, height = 6)
grid.arrange(p1,p3,p2,p4)
dev.off()
